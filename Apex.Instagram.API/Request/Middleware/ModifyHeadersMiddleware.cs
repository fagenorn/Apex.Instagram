...
namespace Apex.Instagram.API.Request.Middleware
{
    ...
    {
        private readonly Dictionary<string, Func<string>> _headers = new Dictionary<string, Func<string>>();
        ...
        public void AddHeader(string key, string name) { AddHeader(key, () => name); }

        public void AddHeader(string key, Func<string> name) { _headers[key] = name; }
